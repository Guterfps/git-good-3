let vegetables = [
    ...
    let Orenge=vegetables.filter(v => v.color==="orange")
    console.log(Orenge)
    ...
    let goodperformance=people.filter(p=>p.goodPerformance===true).forEach(p=>p.salary+=300)
   ...
   let findCommentByID=(postID, commentID)=> posts.find(p=>p.id===postID).comments[commentID]
